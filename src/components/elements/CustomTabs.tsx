        ...
        {tabs.map((tab, index) => (
          <button
            ...
            className={`relative cursor-pointer px-2 pb-2 text-xl transition-all md:text-2xl ${
              activeTab === index
                ? 'text-tertiary-default dark:text-secondary-default font-semibold'
          ...
          >
            ...
            {activeTab === index && (
              <motion.div
                layoutId='underline'
                className='bg-tertiary-default dark:bg-secondary-default absolute -bottom-0.5 left-0 h-0.5 w-full rounded'
              />
            )}
          </button>
        ))}
